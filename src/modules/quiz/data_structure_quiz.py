...
class DataStructureQuiz:
    ...
    def _load_questions(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        try:
            ...
            for idx, row in df.iterrows():  # df.iterrows() 方法返回的是一个包含两个元素的元组：(行索引, 行数据)
                try:
                    # 获取难度值并转换为字符串类型的难度级别
...
def run_quiz() -> None:
    ...
    运行数据结构测验
    """
    quiz = DataStructureQuiz()  # quiz对象
    question_num = 1  # 目前是第几个题目
    ...
    while True:
        ...
        # 获取难度的数值表示
        difficulty_level = difficulty_to_number.get(question['difficulty'],
                                                    lambda: 5)()  # 如果question['difficulty']在字典中存在，返回对应的lambda函数，如果不存在，返回默认值lambda: 5（一个返回5的函数）
